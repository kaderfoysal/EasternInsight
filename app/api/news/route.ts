...
export async function POST(request: NextRequest) {
  try {
    ...
    const slugExists = await News.findOne({ slug: newsSlug });
    if (slugExists) {
      return NextResponse.json(
    ...
    if (typeof priority !== "number") {
      const highestPriorityNews = await News.findOne({
        category: categoryDoc._id,
      })
        .sort({ priority: -1 })
        .select("priority");

      if (highestPriorityNews?.priority) {
        finalPriority = highestPriorityNews.priority + 1;
      } else {
        finalPriority = 1;
      }
    }
