...
export default function AdminCategoryForm({ onCreated }: { onCreated?: () => void }) {
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [serial, setSerial] = useState('' as number | '');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (!serial || serial < 1) {
      setError('Serial number is required and must be greater than or equal to 1');
      setLoading(false);
      return;
    }
    ...
    try {
      const res = await fetch('/api/categories', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, description, serial }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || 'Failed to create category');
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              ...
              min={1}
              value={serial}
              onChange={(e: any) => setSerial(e.target.value === '' ? '' : Number(e.target.value))}
              name="serial"
