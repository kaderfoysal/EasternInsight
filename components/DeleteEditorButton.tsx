...
export default function DeleteEditorButton({ userId, onDelete }: DeleteEditorButtonProps) {
  const [loading, setLoading] = useState(false);

  const handleDelete = async () => {
    if (!confirm('আপনি কি নিশ্চিত যে আপনি এই সম্পাদককে মুছে ফেলতে চান?')) {
      return;
    }
    ...
    try {
      ...
      alert('সম্পাদক মুছে ফেলতে সমস্যা হয়েছে');
    } finally {
      setLoading(false);
    }
  };
